...
{#if child != null}
	{#await child then}
		<ul class="text-2xl m-3">
			<Interprog input={tasks} let:task>
				<li slot="finished">
					<svg
				...
				<li slot="errored" let:error>
					...
					<s>{task}</s>
					<span class="ml-2 p-2 bg-error text-error-content rounded-box">{error}</span>
				</li>
				<li slot="spinner">
					<svg
				...
				<li slot="bar" let:finished let:total>
					<svg
						class="h-12 w-12 inline fill-warning"
						enable-background="new 0 0 50 50"
						version="1.1"
					...
					{task}
					<progress class="progress progress-primary w-56 h-4" value={finished} max={total} />
				</li>
			</Interprog>
		</ul>
	{/await}
